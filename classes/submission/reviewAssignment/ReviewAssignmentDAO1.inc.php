...
class ReviewAssignmentDAO extends PKPReviewAssignmentDAO {
	...
	function getRecomendationScore($journalId, $msc) {
		...
		$result =& $this->retrieve(
			'SELECT 
				...
				) as interests, CASE WHEN tt.incomplete IS NULL THEN 0 ELSE tt.incomplete END as incomplete
			FROM 
				users u 
		...
		while (!$result->EOF) {
			$row = $result->GetRowAssoc(false);
			$recomendationScore[$row['user_id']] = $this->getScore($msc, $row['interests']) * (1 - ($row['incomplete'] / $submissionsCount[1]));
			$result->MoveNext();
		}
	...
	function &getScore($msc, $interests){
		if($msc == '') return 0;
		$score = 0;
		$scoring = array();
		$msc = preg_split('/[\s;,]+/', $msc, -1, PREG_SPLIT_NO_EMPTY);
		$interests = preg_split('/[\s;,]+/', $interests, -1, PREG_SPLIT_NO_EMPTY);
		foreach($msc as $code){
			foreach($interests as $interest){
				if(substr($interest, 0, strlen($code)) === $code){
					$score++;
					$scoring[] = 1;
					break;
				}
				if(substr($interest, 0, strlen(substr($code, 0, -1))) === substr($code, 0, -1)){
					$score+=0.8;
					$scoring[] = 0.8;
					break;
				}
				if(substr($interest, 0, strlen(substr($code, 0, -2))) === substr($code, 0, -2)){
					$score+=0.6;
					$scoring[] = 0.6;
					break;
				}
				if(substr($interest, 0, strlen(substr($code, 0, -3))) === substr($code, 0, -3)){
					$score+=0.4;
					$scoring[] = 0.4;
					break;
				}			
		...
		$k = count($scoring);
		foreach(array_count_values($scoring) as $value => $count){
			$sc = $value*$count;
			
		}
		$sc=$sc/count($msc);
		$score = $score/count($msc);
