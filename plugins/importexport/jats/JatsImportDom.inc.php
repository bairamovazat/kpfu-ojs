...
/**
 * @file plugins/importexport/jats/JatsImportDom.inc.php
 *
 ...
 *
 * @brief Articulus import/export plugin DOM functions for import
 */
...
class JatsImportDom {
	function importArticles(&$journal, &$nodes, &$issue, &$section, &$articles, &$errors, &$user, $isCommandLine) {
		...
		foreach ($nodes as $node) {
			$result = JatsImportDom::handleArticleNode($journal, $node, $issue, $section, $article, $publishedArticle, $articleErrors, $user, $isCommandLine, $dependentItems);
			if ($result) {
				$articles[] = $article;
			} else {
		...
		if ($hasErrors) {
			JatsImportDom::cleanupFailure ($dependentItems);
			return false;
		}
		return true;
	}
	...
	function importArticle(&$journal, &$node, &$issue, &$section, &$article, &$errors, &$user, $isCommandLine) {
		$dependentItems = array();
		$result = JatsImportDom::handleArticleNode($journal, $node, $issue, $section, $article, $publishedArticle, $errors, $user, $isCommandLine, $dependentItems);
		if (!$result) {
			JatsImportDom::cleanupFailure ($dependentItems);
		}
		return $result;
	...
	function importIssues(&$journal, &$issueNodes, &$issues, &$errors, &$user, $isCommandLine) {
		...
		foreach ($issueNodes as $issueNode) {
			$result = JatsImportDom::importIssue($journal, $issueNode, $issue, $issueErrors, $user, $isCommandLine, $dependentItems, false);
			if ($result) {
	...
	function importIssue(&$journal, &$issueNode, &$issue, &$errors, &$user, $isCommandLine, &$dependentItems, $cleanupErrors = true) {
		...
		/* --- Set IDs --- */
		if (!JatsImportDom::handlePubIds($issueNode, $issue, $journal, $issue, $article, $errors)) $hasErrors = true;
		...
		for ($index=0; ($node = $issueNode->getChildByName('title', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.issueTitleLocaleUnsupported', array('issueTitle' => $node->getValue(), 'locale' => $locale));
				$hasErrors = true;
				continue;
			}
		...
		for ($index=0; ($node = $issueNode->getChildByName('description', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.issueDescriptionLocaleUnsupported', array('issueTitle' => $issue->getLocalizedTitle(), 'locale' => $locale));
				$hasErrors = true;
				continue;
			}
		...
		if (($node = $issueNode->getChildByName('date_published'))) {
			...
			if ($publishedDate === -1) {
				$errors[] = array('plugins.importexport.jats.import.error.invalidDate', array('value' => $node->getValue()));
				if ($cleanupErrors) {
					JatsImportDom::cleanupFailure ($dependentItems);
				}
				return false;
		...
		switch(($value = $issueNode->getAttribute('identification'))) {
			...
			case null:
				...
				break;
			default:
				$errors[] = array('plugins.importexport.jats.import.error.unknownIdentificationType', array('identificationType' => $value, 'issueTitle' => $issue->getLocalizedTitle()));
				$hasErrors = true;
				break;
		}

		if (($issueNode->getAttribute('identification') == 'title' || $issueNode->getAttribute('identification') == '') && (!$titleExists)) {
			$errors[] = array('plugins.importexport.jats.import.error.titleMissing', array());
			// Set a placeholder title so that further errors are
			// somewhat meaningful; this placeholder will not be
			// inserted into the database.
			$issue->setTitle(__('plugins.importexport.jats.import.error.defaultTitle'), $journalPrimaryLocale);
			$hasErrors = true;
		}
		...
		switch(($value = $issueNode->getAttribute('published'))) {
			...
			case null:
				...
				break;
			default:
				$errors[] = array('plugins.importexport.jats.import.error.invalidBooleanValue', array('value' => $value));
				$hasErrors = true;
				break;
		}
		...
		if (($value = $issueNode->getAttribute('public_id')) != '') {
			...
			if ($anotherIssue) {
				$errors[] = array('plugins.importexport.jats.import.error.duplicatePublicIssueId', array('issueTitle' => $issue->getIssueIdentification(), 'otherIssueTitle' => $anotherIssue->getIssueIdentification()));
				$hasErrors = true;
			} else {
		...
		for ($index = 0; ($node = $issueNode->getChildByName('cover', $index)); $index++) {
			if (!JatsImportDom::handleCoverNode($journal, $node, $issue, $coverErrors, $isCommandLine)) {
		...
		for ($index = 0; ($node = $issueNode->getChildByName('section', $index)); $index++) {
			if (!JatsImportDom::handleSectionNode($journal, $node, $issue, $sectionErrors, $user, $isCommandLine, $dependentItems, $index)) {
	...
	function handleCoverNode(&$journal, &$coverNode, &$issue, &$errors, $isCommandLine) {
		...
		if ($locale == '') {
			$locale = $journalPrimaryLocale;
				...
				$errors[] = array('plugins.importexport.jats.import.error.coverLocaleUnsupported', array('issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false;
		}
		...
		if (($node = $coverNode->getChildByName('image'))) {
			...
			if (($href = $node->getChildByName('href'))) {
				...
				if ($isCommandLine || JatsImportDom::isAllowedMethod($url)) {
					if ($isCommandLine && JatsImportDom::isRelativePath($url)) {
					...
					if (!$publicFileManager->copyJournalFile($journal->getId(), $url, $newName)) {
						$errors[] = array('plugins.importexport.jats.import.error.couldNotCopy', array('url' => $url));
						$hasErrors = true;
					}
			...
			if (($embed = $node->getChildByName('embed'))) {
				if (($type = $embed->getAttribute('encoding')) !== 'base64') {
					$errors[] = array('plugins.importexport.jats.import.error.unknownEncoding', array('type' => $type));
					$hasErrors = true;
				} else {
					...
					if ($publicFileManager->writeJournalFile($journal->getId(), $newName, base64_decode($embed->getValue()))===false) {
						$errors[] = array('plugins.importexport.jats.import.error.couldNotWriteFile', array('originalName' => $originalName));
						$hasErrors = true;
					}
				}
			}
	...
	function isRelativePath($url) {
		...
		if (JatsImportDom::isAllowedMethod($url)) return false;
	...
	function handleSectionNode(&$journal, &$sectionNode, &$issue, &$errors, &$user, $isCommandLine, &$dependentItems, $sectionIndex = null) {
		...
		for ($index=0; ($node = $sectionNode->getChildByName('title', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.sectionTitleLocaleUnsupported', array('sectionTitle' => $node->getValue(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false; // or ignore this error?
			}
			$titles[$locale] = $node->getValue();
		}
		if (empty($titles)) {
			$errors[] = array('plugins.importexport.jats.import.error.sectionTitleMissing', array('issueTitle' => $issue->getIssueIdentification()));
			return false;
		}
		...
		for ($index=0; ($node = $sectionNode->getChildByName('abbrev', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.sectionAbbrevLocaleUnsupported', array('sectionAbbrev' => $node->getValue(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false; // or ignore this error?
			}
		...
		for ($index=0; ($node = $sectionNode->getChildByName('identify_type', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.sectionIdentifyTypeLocaleUnsupported', array('sectionIdentifyType' => $node->getValue(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false; // or ignore this error?
			}
		...
		for ($index=0; ($node = $sectionNode->getChildByName('policy', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.sectionPolicyLocaleUnsupported', array('sectionPolicy' => $node->getValue(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false; // or ignore this error?
			}
			...
			if ($section) {
				...
				if ($foundSectionId) {
					if ($foundSectionId != $sectionId) {
						...
						$errors[] = array('plugins.importexport.jats.import.error.sectionTitleMismatch', array('section1Title' => $title, 'section2Title' => $foundSectionTitle, 'issueTitle' => $issue->getIssueIdentification()));
						return false;
					}
				} else if ($index > 0) {
						// the current title matches, but the prev titles didn't => error
						$errors[] = array('plugins.importexport.jats.import.error.sectionTitleMatch', array('sectionTitle' => $title, 'issueTitle' => $issue->getIssueIdentification()));
						return false;
				}
				...
				if ($foundSectionId) {
					...
					$errors[] = array('plugins.importexport.jats.import.error.sectionTitleMatch', array('sectionTitle' => $foundSectionTitle, 'issueTitle' => $issue->getIssueIdentification()));
					return false;
				}
			}
			$index++;
		...
		foreach($abbrevs as $locale => $abbrev) {
			...
			if ($abbrevSection) {
				...
				if ($foundSectionId) {
					if ($foundSectionId != $sectionId) {
						// Mismatching sections found. Throw an error.
						$errors[] = array('plugins.importexport.jats.import.error.sectionAbbrevMismatch', array('section1Abbrev' => $abbrev, 'section2Abbrev' => $foundSectionAbbrev, 'issueTitle' => $issue->getIssueIdentification()));
						return false;
					}
				} else if ($index > 0) {
					// the current abbrev matches, but the prev abbrevs didn't => error
					$errors[] = array('plugins.importexport.jats.import.error.sectionAbbrevMatch', array('sectionAbbrev' => $sectionAbbrev, 'issueTitle' => $issue->getIssueIdentification()));
					return false;
				}
				$foundSectionId = $sectionId;
				...
				if ($foundSectionId) {
					...
					$errors[] = array('plugins.importexport.jats.import.error.sectionAbbrevMatch', array('sectionAbbrev' => $foundSectionAbbrev, 'issueTitle' => $issue->getIssueIdentification()));
					return false;
				}
			}
			$index++;
		...
		for ($index = 0; ($node = $sectionNode->getChildByName('article', $index)); $index++) {
			if (!JatsImportDom::handleArticleNode($journal, $node, $issue, $section, $article, $publishedArticle, $articleErrors, $user, $isCommandLine, $dependentItems)) {
				$errors = array_merge($errors, $articleErrors);
				$hasErrors = true;
			}
		}
	...
	function handleArticleNode(&$journal, &$articleNode, &$issue, &$section, &$article, &$publishedArticle, &$errors, &$user, $isCommandLine, &$dependentItems) {
		...
		if (($value = $articleNode->getAttribute('public_id')) != '') {
			...
			if ($anotherArticle) {
				$errors[] = array('plugins.importexport.jats.import.error.duplicatePublicArticleId', array('articleTitle' => $article->getLocalizedTitle(), 'otherArticleTitle' => $anotherArticle->getLocalizedTitle()));
				$hasErrors = true;
			} else {
		...
		for ($index=0; ($node = $articleNode->getChildByName('title', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.articleTitleLocaleUnsupported', array('articleTitle' => $node->getValue(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false;
			}
		...
		if (!$titleExists || $article->getTitle($article->getLocale()) == '') {
			$errors[] = array('plugins.importexport.jats.import.error.articleTitleMissing', array('issueTitle' => $issue->getIssueIdentification(), 'sectionTitle' => $section->getLocalizedTitle()));
			return false;
		}
		...
		for ($index=0; ($node = $articleNode->getChildByName('abstract', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.articleAbstractLocaleUnsupported', array('articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false;
			}
		...
		if (($indexingNode = $articleNode->getChildByName('indexing'))) {
			for ($index=0; ($node = $indexingNode->getChildByName('discipline', $index)); $index++) {
				...
				if ($locale == '') {
					...
					$errors[] = array('plugins.importexport.jats.import.error.articleDisciplineLocaleUnsupported', array('articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
					return false;
				}
			...
			for ($index=0; ($node = $indexingNode->getChildByName('type', $index)); $index++) {
				...
				if ($locale == '') {
					...
					$errors[] = array('plugins.importexport.jats.import.error.articleTypeLocaleUnsupported', array('articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
					return false;
				}
			...
			for ($index=0; ($node = $indexingNode->getChildByName('subject', $index)); $index++) {
				...
				if ($locale == '') {
					...
					$errors[] = array('plugins.importexport.jats.import.error.articleSubjectLocaleUnsupported', array('articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
					return false;
				}
			...
			for ($index=0; ($node = $indexingNode->getChildByName('subject_class', $index)); $index++) {
				...
				if ($locale == '') {
					...
					$errors[] = array('plugins.importexport.jats.import.error.articleSubjectClassLocaleUnsupported', array('articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
					return false;
				}
			...
			if (($coverageNode = $indexingNode->getChildByName('coverage'))) {
				for ($index=0; ($node = $coverageNode->getChildByName('geographical', $index)); $index++) {
					...
					if ($locale == '') {
						...
						$errors[] = array('plugins.importexport.jats.import.error.articleCoverageGeoLocaleUnsupported', array('articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
						return false;
					}
				...
				for ($index=0; ($node = $coverageNode->getChildByName('chronological', $index)); $index++) {
					...
					if ($locale == '') {
						...
						$errors[] = array('plugins.importexport.jats.import.error.articleCoverageChronLocaleUnsupported', array('articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
						return false;
					}
				...
				for ($index=0; ($node = $coverageNode->getChildByName('sample', $index)); $index++) {
					...
					if ($locale == '') {
						...
						$errors[] = array('plugins.importexport.jats.import.error.articleCoverageSampleLocaleUnsupported', array('articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
						return false;
					}
		...
		for ($index=0; ($node = $articleNode->getChildByName('sponsor', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.articleSponsorLocaleUnsupported', array('articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false;
			}
		...
		for ($index = 0; ($node = $articleNode->getChildByName('cover', $index)); $index++) {
			if (!JatsImportDom::handleArticleCoverNode($journal, $node, $article, $coverErrors, $isCommandLine)) {
				$errors = array_merge($errors, $coverErrors);
				$hasErrors = true;
			}
		}
		...
		/* --- Set IDs --- */
		if (!JatsImportDom::handlePubIds($articleNode, $article, $journal, $issue, $article, $errors)) $hasErrors = true;
		...
		for ($index = 0; ($node = $articleNode->getChildByName('author', $index)); $index++) {
			if (!JatsImportDom::handleAuthorNode($journal, $node, $issue, $section, $article, $authorErrors, $index)) {
		...
		for ($index=0; $index < count($articleNode->children); $index++) {
			...
			if (!JatsImportDom::handleGalleyNode($journal, $node, $issue, $section, $article, $galleyErrors, $isCommandLine, $isHtml, $galleyCount, $articleFileManager)) {
		...
		for ($index = 0; ($node = $articleNode->getChildByName('supplemental_file', $index)); $index++) {
			if (!JatsImportDom::handleSuppFileNode($journal, $node, $issue, $section, $article, $suppFileErrors, $isCommandLine, $articleFileManager)) {
	...
	function handleAuthorNode(&$journal, &$authorNode, &$issue, &$section, &$article, &$errors, $authorIndex) {
		...
		for ($index=0; ($node = $authorNode->getChildByName('affiliation', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.articleAuthorAffiliationLocaleUnsupported', array('authorFullName' => $author->getFullName(), 'articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false;
			}
		...
		for ($index=0; ($node = $authorNode->getChildByName('competing_interests', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.articleAuthorCompetingInterestsLocaleUnsupported', array('authorFullName' => $author->getFullName(), 'articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false;
			}
		...
		for ($index=0; ($node = $authorNode->getChildByName('biography', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.articleAuthorBiographyLocaleUnsupported', array('authorFullName' => $author->getFullName(), 'articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false;
			}
	...
	function handleGalleyNode(&$journal, &$galleyNode, &$issue, &$section, &$article, &$errors, $isCommandLine, $isHtml, $galleyCount, &$articleFileManager) {
		...
		if (($value = $galleyNode->getAttribute('public_id')) != '') {
			...
			if ($anotherGalley) {
				$errors[] = array('plugins.importexport.jats.import.error.duplicatePublicGalleyId', array('publicId' => $value, 'articleTitle' => $article->getLocalizedTitle()));
				$hasErrors = true;
			} else {
		...
		/* --- Set IDs --- */
		if (!JatsImportDom::handlePubIds($galleyNode, $galley, $journal, $issue, $article, $errors)) return false;
		...
		if ($locale == '') {
			...
			$errors[] = array('plugins.importexport.jats.import.error.galleyLocaleUnsupported', array('articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
			return false;
		}
		...
		if (!($node = $galleyNode->getChildByName('label'))) {
			$errors[] = array('plugins.importexport.jats.import.error.galleyLabelMissing', array('articleTitle' => $article->getLocalizedTitle(), 'sectionTitle' => $section->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification()));
			return false;
		}
		...
		if (!($node = $galleyNode->getChildByName('file'))) {
			$errors[] = array('plugins.importexport.jats.import.error.galleyFileMissing', array('articleTitle' => $article->getLocalizedTitle(), 'sectionTitle' => $section->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification()));
			return false;
		}
		...
		if ($isHtml) {
			$result = JatsImportDom::handleHtmlGalleyNodes($galleyNode, $articleFileManager, $galley, $errors, $isCommandLine);
	...
	function handlePubIds(&$node, &$pubObject, &$journal, &$issue, &$article, &$errors) {
		for ($index=0; ($idNode = $node->getChildByName('id', $index)); $index++) {
			...
			if (is_array($pubIdPlugins)) foreach ($pubIdPlugins as $pubIdPlugin) {
				if ($pubIdPlugin->getPubIdType() == $pubIdType) {
					...
					if (!$pubIdPlugin->validatePubId($pubId)) {
						$errors[] = array('plugins.importexport.jats.import.error.invalidPubId', $errorParams);
						return false;
					}
					if (!$pubIdPlugin->checkDuplicate($pubId, $pubObject, $journal->getId())) {
						$errors[] = array('plugins.importexport.jats.import.error.duplicatePubId', $errorParams);
						return false;
					}
			...
			if (!$pubIdPluginFound) {
				$errors[] = array('plugins.importexport.jats.import.error.unknownPubId', $errorParams);
				return false;
			}
		}
		return true;
	...
	function handleSuppFileNode(&$journal, &$suppNode, &$issue, &$section, &$article, &$errors, $isCommandLine, &$articleFileManager) {
		...
		/* --- Set IDs --- */
		if (!JatsImportDom::handlePubIds($suppNode, $suppFile, $journal, $issue, $article, $errors)) return false;
		...
		for ($index=0; ($node = $suppNode->getChildByName('title', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.articleSuppFileTitleLocaleUnsupported', array('suppFileTitle' => $node->getValue(), 'articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false;
			}
		...
		for ($index=0; ($node = $suppNode->getChildByName('creator', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.articleSuppFileCreatorLocaleUnsupported', array('suppFileTitle' => $node->getValue(), 'articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false;
			}
		...
		for ($index=0; ($node = $suppNode->getChildByName('subject', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.articleSuppFileSubjectLocaleUnsupported', array('suppFileTitle' => $node->getValue(), 'articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false;
			}
		...
		for ($index=0; ($node = $suppNode->getChildByName('type_other', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.articleSuppFileTypeOtherLocaleUnsupported', array('suppFileTitle' => $node->getValue(), 'articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false;
			}
		...
		for ($index=0; ($node = $suppNode->getChildByName('description', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.articleSuppFileDescriptionLocaleUnsupported', array('suppFileTitle' => $node->getValue(), 'articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false;
			}
		...
		for ($index=0; ($node = $suppNode->getChildByName('publisher', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.articleSuppFilePublisherLocaleUnsupported', array('suppFileTitle' => $node->getValue(), 'articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false;
			}
		...
		for ($index=0; ($node = $suppNode->getChildByName('sponsor', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.articleSuppFileSponsorLocaleUnsupported', array('suppFileTitle' => $node->getValue(), 'articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false;
			}
		...
		for ($index=0; ($node = $suppNode->getChildByName('source', $index)); $index++) {
			...
			if ($locale == '') {
				...
				$errors[] = array('plugins.importexport.jats.import.error.articleSuppFileSourceLocaleUnsupported', array('suppFileTitle' => $node->getValue(), 'articleTitle' => $article->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification(), 'locale' => $locale));
				return false;
			}
		...
		switch (($suppType = $suppNode->getAttribute('type'))) {
			...
			default:
				$errors[] = array('plugins.importexport.jats.import.error.unknownSuppFileType', array('suppFileType' => $suppType));
				return false;
		}
		...
		if (($value = $suppNode->getAttribute('public_id')) != '') {
			...
			if ($anotherSuppFile) {
				$errors[] = array('plugins.importexport.jats.import.error.duplicatePublicSuppFileId', array('suppFileTitle' => $suppFile->getLocalizedTitle(), 'otherSuppFileTitle' => $anotherSuppFile->getLocalizedTitle()));
				$hasErrors = true;
			} else {
		...
		if (!($fileNode = $suppNode->getChildByName('file'))) {
			$errors[] = array('plugins.importexport.jats.import.error.suppFileMissing', array('articleTitle' => $article->getLocalizedTitle(), 'sectionTitle' => $section->getLocalizedTitle(), 'issueTitle' => $issue->getIssueIdentification()));
			return false;
		}
