...
define('ARTICULUS_DTD_ID', '-//NLM//DTD JATS (Z39.96) Journal Publishing DTD v1.1d1 20130915//EN');

class JatsImportExportPlugin extends ImportExportPlugin {
	/**
	 * Constructor
	 */
	function JatsImportExportPlugin() {
	...
	function getDTDUrl() {
		...
		return 'http://pkp.sfu.ca/ojs/dtds/' . urlencode($currentVersion->getMajor() . '.' . $currentVersion->getMinor() . '.' . $currentVersion->getRevision()) . '/jats.dtd';
	}
	...
	function display(&$args, $request) {
		...
		switch (array_shift($args)) {
			...
			case 'import':
				...
				if (!$temporaryFile) {
					$templateMgr->assign('error', 'plugins.importexport.jats.error.uploadFailed');
	...
	function exportIssue(&$journal, &$issue, $outputFile = null) {
		...
		$doc =& XMLCustomWriter::createDocument(/*'issue', ARTICULUS_DTD_ID, $this->getDTDUrl()*/);
	...
	function exportArticle(&$journal, &$issue, &$section, &$article, $outputFile = null) {
		$this->import('JatsExportDom');
		$doc =& XMLCustomWriter::createDocument('article', ARTICULUS_DTD_ID, "JATS-journalpublishing1.dtd");
	...
	function exportIssues(&$journal, &$issues, $outputFile = null) {
		$this->import('JatsExportDom');
		$doc =& XMLCustomWriter::createDocument(/*'issues', ARTICULUS_DTD_ID, $this->getDTDUrl()*/);
	...
	function exportArticles(&$results, $outputFile = null) {
		$this->import('JatsExportDom');
		$doc =& XMLCustomWriter::createDocument(/*'articles', ARTICULUS_DTD_ID, $this->getDTDUrl()*/);
	...
	function handleImport(&$context, &$doc, &$errors, &$issues, &$articles, $isCommandLine) {
		...
		switch ($rootNodeName) {
			case 'issues':
				return ArticulusImportDom::importIssues($journal, $doc->children, $issues, $errors, $user, $isCommandLine);
				break;
			case 'issue':
				$dependentItems = array();
				$result = ArticulusImportDom::importIssue($journal, $doc, $issue, $errors, $user, $isCommandLine, $dependentItems);
			...
			case 'articles':
				...
				$issue =& $context['issue'];
				return ArticulusImportDom::importArticles($journal, $doc->children, $issue, $section, $articles, $errors, $user, $isCommandLine);
				break;
			case 'article':
				$section =& $context['section'];
				$issue =& $context['issue'];
				$result = ArticulusImportDom::importArticle($journal, $doc, $issue, $section, $article, $errors, $user, $isCommandLine);
				if ($result) $articles = array($article);
				return $result;
				break;
			default:
				$errors[] = array('plugins.importexport.jats.import.error.unsupportedRoot', array('rootName' => $rootNodeName));
				return false;
				break;
		}
	}
		...
		switch ($command) {
			case 'import':
				...
				if (!$user) {
					if ($userName != '') {
						echo __('plugins.importexport.jats.cliError') . "\n";
						echo __('plugins.importexport.jats.error.unknownUser', array('userName' => $userName)) . "\n\n";
					}
					$this->usage($scriptName);
					return;
				}
				...
				switch ($this->getRootNodeName($doc)) {
					...
					case 'articles':
						...
						if (!$section) {
							echo __('plugins.importexport.jats.cliError') . "\n";
							echo __('plugins.importexport.jats.export.error.sectionNotFound', array('sectionIdentifier' => $sectionIdentifier)) . "\n\n";
							return;
						}
				...
				if ($result) {
					echo __('plugins.importexport.jats.import.success.description') . "\n\n";
