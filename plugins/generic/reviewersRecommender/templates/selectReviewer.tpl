{**
 * templates/reviewersRecommender/selectReviewer.tpl
 *
...
<p>{translate key="article.subjectClassification"}: {$msc}</p>
...
<tr class="heading" valign="bottom">
	...
	<td width="25%">{translate key="user.interests"}</td>
	<td width="6%">{sort_search key="plugins.generic.revrec.score" sort="recommendationScore"}</td>
	{if $rateReviewerOnQuality}
		<td width="6%">{sort_search key="reviewer.averageQuality" sort="quality"}</td>
	{/if}
	<td width="6%">{sort_search key="reviewer.completedReviews" sort="done"}</td>
	<td width="6%">{sort_search key="editor.submissions.averageTime" sort="average"}</td>
	<td width="10%">{sort_search key="editor.submissions.lastAssigned" sort="latest"}</td>
	<td width="5%">{sort_search key="common.active" sort="active"}</td>
	<td width="6%" class="heading">{translate key="common.action"}</td>
</tr>
<tr><td colspan="{$numCols|escape}" class="headseparator">&nbsp;</td></tr>
...
<tr valign="top">
	...
	<td>{$reviewer->interest}</td>
	<td>{if $reviewer->score}<progress max="1" value="{$reviewer->score|replace:",":"."}">{$reviewer->score|replace:",":"."}</progress><p>{$reviewer->score*100|string_format:"%.1f"}%</p>{/if}</td>
...
<tr><td colspan="{$numCols|escape}" class="{if $reviewer==end($reviewers)}end{/if}separator">&nbsp;</td></tr>
{/foreach}
