...
class RevUserManagementForm extends Form {
	 ...
	 * Constructor.
	 */
	function RevUserManagementForm($userId = null) {
		$revrecPlugin =& PluginRegistry::getPlugin('generic', REVREC_PLUGIN_NAME);
		parent::Form($revrecPlugin->getTemplatePath() . 'userProfileForm.tpl', true);
		...
		if (isset($this->userId)) {
			...
			if ($user != null) {
				$this->_data = array(
					...
					'mathscinet' => $user->getData('mathscinet'),
					'zbmath' => $user->getData('zbmath'),
					'userUrl' => $user->getUrl(),
					'phone' => $user->getPhone(),
					'fax' => $user->getFax(),
	...
	function execute() {
		...
		$user->setData('mathscinet', $this->getData('mathscinet'));
		$user->setData('zbmath', $this->getData('zbmath'));
